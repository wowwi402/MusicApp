...
const PLAYLITS_KEY = 'favorites:list';
...
export default function FavoritesScreen() {
  ...
  const load = async () => {
    try {
      const raw = await AsyncStorage.getItem(PLAYLITS_KEY);
  ...
  const removeOne = async (id: string) => {
    ...
    await AsyncStorage.setItem(PLAYLITS_KEY, JSON.stringify(next));
    setItems(next);
  };
  ...
  const clearAll = () => {
    Alert.alert('Xóa tất cả', 'Bạn chắc chắn muốn xóa toàn bộ Favorites?', [
      ...
      { text: 'Xóa', style: 'destructive', onPress: async () => {
        await AsyncStorage.removeItem(PLAYLITS_KEY);
        setItems([]);
      }}
    ]);
  };
